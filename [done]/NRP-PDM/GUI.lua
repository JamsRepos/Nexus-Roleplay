...
GUI.maxVisOptions = 28
GUI.titleText = {13, 191, 16, 250, 7}
GUI.titleRect = {0, 0, 0, 255}
GUI.optionText = {0, 0, 0, 250, 4}
GUI.optionRect = {40, 40, 40, 200}
GUI.scroller = {13, 191, 16, 255}
...
function Menu.Option(option, select)
	...
	if not select then
		if(currentOption <= GUI.maxVisOptions and optionCount <= GUI.maxVisOptions) then
			...
			if(thisOption) then
				GUI.Text(option, {0, 0, 0, 255, 6}, {menuX, optionCount * 0.035 + 0.125},  {0.5, 0.5 }, true)
		...
		if(currentOption <= GUI.maxVisOptions and optionCount <= GUI.maxVisOptions) then
			...
			if(thisOption) then
				GUI.Text(option, {0, 0, 0, 255, 6}, {menuX, optionCount * 0.035 + 0.125},  {0.5, 0.5 }, true)
				GUI.Rect({255, 255, 0, 255}, { menuX, optionCount * 0.035 + 0.1415 }, { 0.23, 0.035 })
			end
		elseif (optionCount > currentOption - GUI.maxVisOptions and optionCount <= currentOption) then
...
function Menu.OptionData(option, text)
	...
	if(currentOption <= GUI.maxVisOptions and optionCount <= GUI.maxVisOptions) then
		...
		GUI.Text(NumberFormat(text), {0, 99, 2,255, 6}, { menuX + 0.085, optionCount * 0.035 + 0.125 }, { 0.5, 0.5 }, true)
	elseif(optionCount > currentOption - GUI.maxVisOptions and optionCount <= currentOption) then
		
		GUI.Text(NumberFormat(text), {0, 0, 0, 255, 6}, { menuX + 0.085, optionCount - (currentOption - GUI.maxVisOptions) * 0.035 + 0.125 }, { 0.5, 0.5 }, true)
	end
