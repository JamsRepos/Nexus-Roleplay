...
RegisterServerEvent('housing:vault:getVault')
AddEventHandler('housing:vault:getVault', function(id, storage)
 local source = tonumber(source)
 local storageitems = {}
 local result = exports['GHMattiMySQL']:QueryResult("SELECT * FROM `stored_inventorys` WHERE unique_id=@unique_id",{['@unique_id'] = 'vault-'..id}) 
 for _,v in pairs(result) do
  table.insert(storageitems, {id = v.id, name = v.name, item = v.item, q = v.qty, meta = v.meta})
 end
 ipl_storage = storage
 TriggerClientEvent('housing:vault:updateitems', source, storageitems, ipl_storage)
 TriggerClientEvent('housing:vault:openInventory', source, id, ipl_storage)
end)

RegisterServerEvent('housing:vault:additems')
AddEventHandler('housing:vault:additems', function(id, item, name, qty, metadata)
 local source = tonumber(source)
 TriggerEvent('core:getPlayerFromId', source, function(user)
  canGive = user.isAbleToGive(item,qty,metadata)
  if canGive then
   user.removeQuantity(item,qty,metadata)
    exports['GHMattiMySQL']:QueryAsync('INSERT INTO `stored_inventorys` (unique_id, item, name, qty, meta) VALUES (@unique_id,@itemid,@item,@qty, @meta) ON DUPLICATE KEY UPDATE qty=qty+@qty',{['@unique_id'] = 'vault-'..id, ['@qty'] = qty, ['@item'] = name, ['@itemid'] = item, ['@meta'] = metadata})
    TriggerEvent('housing:vault:refresh', source, id)
  end
 end)
end)

RegisterServerEvent('housing:vault:removeitems')
AddEventHandler('housing:vault:removeitems', function(id, item, qty, meta)
 local source = tonumber(source)
 TriggerEvent('core:getPlayerFromId', source, function(user)
  local canGet = user.isAbleToReceive(qty)
  if not canGet then
   ...
   user.addQuantity(item,qty, meta)
   exports['GHMattiMySQL']:QueryAsync('UPDATE `stored_inventorys` SET `qty`= `qty` - @qty WHERE `unique_id` = @unique_id AND `item`= @item and `meta` = @meta',{['@unique_id'] = 'vault-'..id, ['@qty'] = qty, ['@item'] = item, ['@meta'] = meta})
   TriggerEvent('housing:vault:refresh', source, id)
  end
 end)
end)

RegisterServerEvent('housing:vault:refresh')
AddEventHandler('housing:vault:refresh', function(source, id)
 local source = tonumber(source)
 local storageitems = {}
 local result = exports['GHMattiMySQL']:QueryResult("SELECT * FROM `stored_inventorys` WHERE unique_id=@unique_id",{['@unique_id'] = 'vault-'..id}) 
 for _,v in pairs(result) do
