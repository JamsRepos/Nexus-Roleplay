...
function tryTakeChips(source,amount)
    ...
    TriggerEvent('core:getPlayerFromId', source, function(user)
      if tonumber(amount) <= 5000 then
        if user.getMoney() >= tonumber(amount) and tonumber(amount) >= 1 and tonumber(amount) <= 5000 then
          user.removeMoney(amount)
          TriggerClientEvent('NRP-notify:client:SendAlert', source, { type = 'success', text = "You Bet $ "..amount.." GoodLuck!"})
          bet = true
        else
          TriggerClientEvent('NRP-notify:client:SendAlert', source, { type = 'error', text = "You Dont Have Enough Cash To Make The Bet!"})
          bet = false
        end
      else
        TriggerClientEvent('NRP-notify:client:SendAlert', source, { type = 'error', text = "Max Bet: 5000$!"})
      end
    end) 
  if bet == true then 
    return true
...
function giveChips(source,amount)
    ...
    TriggerEvent('core:getPlayerFromId', source, function(user)
        ...
        TriggerClientEvent('NRP-notify:client:SendAlert', source, { type = 'success', text = "Congratulations you won $ "..amount.."!"})
