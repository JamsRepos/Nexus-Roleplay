    ...
    if ( IsPedSittingInAnyVehicle( ped ) ) then 
        ...
        if ( GetVehicleClass( vehicle ) == 18 ) then
            ...
            if ( radarEnabled ) then 
                TriggerEvent('chatMessage', "POLICE", {74, 146, 247}, "Radar enabled.")
            else 
                TriggerEvent('chatMessage', "POLICE", {74, 146, 247}, "Radar disabled.")
            end 
            ...
            TriggerEvent('chatMessage', "POLICE", {74, 146, 247}, "Error! You must be in a police vehicle.")
        end 
    else 
        TriggerEvent('chatMessage', "POLICE", {74, 146, 247}, "Error! You must be in a police vehicle.")
    end 
...
function ToggleSpeedType()
    if ( radarInfo.speedType == "mph" ) then 
        ...
        TriggerEvent('chatMessage', "POLICE", {74, 146, 247}, "Speed type has been updated to Km/h")
    else 
        radarInfo.speedType = "mph"
        TriggerEvent('chatMessage', "POLICE", {74, 146, 247}, "Speed type has been updated to MPH")
    end
end 
...
function ToggleLockBeep()
    if ( radarInfo.lockBeep ) then 
        ...
        TriggerEvent('chatMessage', "POLICE", {74, 146, 247}, "Radar bleep has been disabled.")
    else 
        radarInfo.lockBeep = true
        TriggerEvent('chatMessage', "POLICE", {74, 146, 247}, "Radar bleep has been enabled.")
    end    
end 
...
Citizen.CreateThread( function()
    while true do 
        ...
        if ( not IsDisabledControlPressed( 1, 36 ) and IsDisabledControlJustPressed( 1, 244 ) ) then
            if DecorGetInt(GetPlayerPed(-1), "Job") == 1 or DecorGetInt(GetPlayerPed(-1), "Job") == 18 or DecorGetInt(GetPlayerPed(-1), "Job") == 31 or DecorGetInt(GetPlayerPed(-1), "Job") == 32 or DecorGetInt(GetPlayerPed(-1), "Job") == 33 or DecorGetInt(GetPlayerPed(-1), "Job") == 34 or DecorGetInt(GetPlayerPed(-1), "Job") == 35 or DecorGetInt(GetPlayerPed(-1), "Job") == 36 or DecorGetInt(GetPlayerPed(-1), "Job") == 37 or user.getJob() == 90 or user.getJob() == 91 then
             TriggerEvent('chatMessage', "POLICE", {74, 146, 247}, "Your speed lock has been reset.")
             ResetFrontFast()
             ResetRearFast()
            end
        end 
