RegisterNetEvent('NRP-notify:client:SendAlert')
AddEventHandler('NRP-notify:client:SendAlert', function(data)
	DoCustomHudText(data.type, data.text, data.length, data.style)
end)

RegisterNetEvent('NRP-notify:client:PersistentHudText')
AddEventHandler('NRP-notify:client:PersistentHudText', function(data)
