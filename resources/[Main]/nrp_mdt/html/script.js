...
function processSuspect() {
 ...
 $.post('http://nrp_mdt/processSuspect', JSON.stringify({id: $("#playerID").val(), time: $("#jailTime").val(), fine: $("#fine").val(), charges: $("#suspectChargeList").val(), isWarning: checkBox.checked})); 
 $("#playerID").val(0);
 $("#jailTime").val(0);
 $("#fine").val(0);
...
function submitNewWarrant() {
 $.post('http://nrp_mdt/newWarrant', JSON.stringify({name: $("#warrantName").val(), vehicles: $("#warrantVehicles").val(), description: $("#warrantDecription").val(), charges: $("#warrantCharges").val()})); 
 $("#warrantName").val('');
 $("#warrantVehicles").val('');
...
function deleteWarrant() {
 $.post('http://nrp_mdt/deleteWarrant', JSON.stringify({id: currentWarrant})); 
}

function searchNCIC(search) {
 if (search == "Person") {
  $.post('http://nrp_mdt/searchNCIC', JSON.stringify({id: 1, firstname: $("#ncicSearchFirstname").val(), lastname: $("#ncicSearchLastname").val()})); 
 } else if (search == "Vehicle") {
  $.post('http://nrp_mdt/searchNCIC', JSON.stringify({id: 2, plate: $("#ncicSearchPlate").val()}));  
 } else if (search == "License") {
  $.post('http://nrp_mdt/searchNCIC', JSON.stringify({id: 4, license: $("#ncicSearchLicense").val()})); 
 }
} 
...
function updateTags(cid) {
 $.post('http://nrp_mdt/updateTags', JSON.stringify({id: cid, status: document.getElementById(cid).checked})); 
}
...
function acceptCall() {
 $.post('http://nrp_mdt/acceptCall', JSON.stringify({id: currentCall})); 
}

function finishCall() {
 $.post('http://nrp_mdt/finishCall', JSON.stringify({id: currentCall})); 
 currentCall = 0;
}

function newCallReport() {
 $.post('http://nrp_mdt/newCallReport', JSON.stringify({name: $("#callName").val(), info: $("#additionalInfo").val(), incident: $("#incidentReport").val()})); 
 $("#callName").val('');
 $("#additionalInfo").val('');
 $("#incidentReport").val('');
}