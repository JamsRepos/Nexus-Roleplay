    ...
    window.addEventListener('message', function (event) {
        ...
        if (data.action == "ui") {
            ...
            if (data.toggle) {
                ...
                setTimeout(function(){
					setCharactersList()
                    $.post('https://nrp-characters/setupCharacters');
                    setTimeout(function(){
...
$('.disconnect-btn').click(function(e){
    ...
    $.post('https://nrp-characters/closeUI');
    $.post('https://nrp-characters/disconnectButton');
});
...
$(".btn-iniciar").on("click", function() {
    $(".title-screen").fadeOut(300, function() {
        ...
        $('.jugadores-on').fadeIn();
        $('.fondocolor').hide();
        qbMultiCharacters.fadeInDown('.imagenlogo', '6vh', 1700);
        $.post('https://nrp-characters/removeBlur');
    })
});
...
function setupCharacters(characters) {
    $.each(characters, function(index, char){
        ...
        setTimeout(function(){
            $('#char-'+char.cid).html('<span id="slot-name"> <i class="fa fa-user" aria-hidden="true" style="color:rgb(255, 255, 255); padding-top: 3px;"></i> '+char.charinfo.firstname+' '+char.charinfo.lastname+'<span id="cid">' + char.citizenid + '</span></span>');
...
$(document).on('click', '.character', function(e) {
    ...
    if (selectedChar === null) {
        ...
        if ((selectedChar).data('cid') == "") {
            ...
            setupCharInfo('empty')
            $("#play-text").html('<i class="fa fa-plus" aria-hidden="true" style="padding-top: 3px"></i> Register');
            $("#play").css({"display":"block"});
            $("#delete").css({"display":"none"});
            $.post('https://nrp-characters/cDataPed', JSON.stringify({
                cData: cDataPed
            }));
        } else {
            $(selectedChar).addClass("char-selected");
            setupCharInfo($(this).data('cData'))
            $("#play-text").html('<i class="fa fa-sign-in" aria-hidden="true" style="padding-top: 3px"></i> Play');
            $("#delete-text").html('<i class="fa fa-trash" aria-hidden="true" style="padding-top: 3px"></i> Delete');
            $("#play").css({"display":"block"});
...
$(document).on('click', '#create', function (e) {
    ...
    $.post('https://nrp-characters/createNewCharacter', JSON.stringify({
        firstname: firstname,
        lastname: lastname,
...
$(document).on('click', '#accept-delete', function(e){
    $.post('https://nrp-characters/removeCharacter', JSON.stringify({
...
$(document).on('click', '#play', function(e) {
    ...
    if (selectedChar !== null) {
        if (charData !== "") {
            $.post('https://nrp-characters/selectCharacter', JSON.stringify({
