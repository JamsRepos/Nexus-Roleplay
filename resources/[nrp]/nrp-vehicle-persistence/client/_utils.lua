...
_Utils.SetVehicleProperties = function(vehicle, props)
	if DoesEntityExist(vehicle) then
		...
		if props.tankHealth then GetVehiclePetrolTankHealth(vehicle, props.tankHealth + 0.0) end
...
_Utils.takeControlOfEntity = function(entity)
...
_Utils.IsDuplicateVehicle = function(plate)
	for vehicle in EnumerateVehicles() do
		if _Utils.Trim(GetVehicleNumberPlateText(vehicle)) == _Utils.Trim(plate) then
			return vehicle
		end
	end
	return false
...
_Utils.DistanceBetweenCoords = function (coordsA, coordsB)
