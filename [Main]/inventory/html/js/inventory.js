...
window.addEventListener("message", function (event) {
    ...
    } else if (event.data.action == "setStorageText") {
        $(".storage-div").html(event.data.text);
...
$(document).ready(function () {
    ...
    $('#playerInventory').droppable({
        drop: function (event, ui) {
            ...
            } else if (type === "medical" && itemInventory === "second") {
                disableInventory(500);
                $.post("http://inventory/TakeFromMedical", JSON.stringify({
                    item: itemData,
                    number: parseInt($("#count").val())
                }));
            } else if (type === "evidence" && itemInventory === "second") {
                disableInventory(500);
                $.post("http://inventory/TakeFromEvidence", JSON.stringify({
                    item: itemData,
                    number: parseInt($("#count").val())
                }));
    ...
    $('#otherInventory').droppable({
        drop: function (event, ui) {
            ...
            } else if (type === "evidence" && itemInventory === "main") {
                disableInventory(500);
                $.post("http://inventory/PutIntoEvidence", JSON.stringify({
                    item: itemData,
                    number: parseInt($("#count").val())
                }));
            } else if (type === "medical" && itemInventory === "main") {
                disableInventory(500);
                $.post("http://inventory/PutIntoMedical", JSON.stringify({
                    item: itemData,
                    number: parseInt($("#count").val())
                }));
