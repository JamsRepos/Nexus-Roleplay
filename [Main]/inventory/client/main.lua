        ...
        if type(data.number) == "number" and math.floor(data.number) == data.number then
         if exports['core']:GetItemQuantity(data.item.id) >= data.number then
           local pos = GetEntityCoords(GetPlayerPed(-1), 0)
            ...
            TriggerServerEvent("core:log", tostring("[DROP] "..GetPlayerName(PlayerId()).."("..PlayerId()..") dropped "..data.number.."x "..data.item.name.."("..data.item.id..") onto the ground."), "item")
         end
        end
...
RegisterNUICallback(
    ...
    if exports['core']:GetItemQuantity(data.item.id) >= data.number then
        ...
        if(distance ~= -1 and distance < 3) then
         ...
         TriggerServerEvent("core:log", tostring("[GIVE] "..GetPlayerName(PlayerId()).."("..PlayerId()..") gave "..data.number.."x "..data.item.name.."("..data.item.id..") to "..GetPlayerName(t).."("..t..")"), "item")
        else
         exports['NRP-notify']:DoHudText('inform', 'No Player Near')
        end
    else
        exports['NRP-notify']:DoHudText('inform', 'You do not have enough of that item to give!')
    end
...
function use(item, meta)
    ...
    if item == 2 then
       ...
       elseif item == 123 or item == 124 or item == 299 then 
        removeQty(item,1)
       ...
       elseif item == 150 then 
        TriggerEvent("NRP-hospital:items:gauze")
        removeQty(item,1)
       elseif item == 152 then 
        TriggerServerEvent('NRP-B-Cashstacks-10k')  
        removeQty(item,1)
       elseif item == 153 then 
        TriggerServerEvent('NRP-B-Cashstacks-5k')  
        removeQty(item,1)
       ...
       elseif item == 172 then
        ...
        AddAmmoToPed(GetPlayerPed(-1), weapon, 50)
        removeQty(item,1)
       elseif item == 177 then 
        TriggerServerEvent('NRP-B-Dirtystacks-5k')  
        removeQty(item,1)
       elseif item == 178 then 
        TriggerServerEvent('NRP-B-Dirtystacks-1k')  
        removeQty(item,1)
       elseif item == 73 then 
        TriggerEvent("NRP-hospital:items:firstaid")
        removeQty(item,1)
       elseif item == 96 then 
       ...
       elseif item == 173 then 
          TriggerEvent("NRP-Mech:Radiator")
          removeQty(item,1)
       ...
       elseif item == 251 then
        ...
        elseif item == 289 then
            if exports['core']:GetItemQuantity(288) >= 1 then
             removeQty(item,1)
             TriggerEvent('fishing:setbait', _source, "fish")
             TriggerEvent('NRP-notify:client:SendAlert', source, { type = 'success', text = 'You attach the fish bait onto your fishing rod!'})
            else
             TriggerEvent('NRP-notify:client:SendAlert', source, { type = 'error', text = "You dont have a fishing rod"})
            end
        elseif item == 290 then
            if exports['core']:GetItemQuantity(288) >= 1 then
             removeQty(item,1)
             TriggerEvent('fishing:setbait', _source, "turtle")
             TriggerEvent('NRP-notify:client:SendAlert', source, { type = 'success', text = 'You attach the turtle bait onto your fishing rod!'})
            else
             TriggerEvent('NRP-notify:client:SendAlert', source, { type = 'error', text = "You dont have a fishing rod"})
            end
        elseif item == 291 then
            TriggerEvent('items:cuff')
        elseif item == 292 then
            if exports['core']:GetItemQuantity(288) >= 1 then
             removeQty(item,1)
             TriggerEvent('fishing:setbait', _source, "shark")
             TriggerEvent('NRP-notify:client:SendAlert', source, { type = 'success', text = 'You attach the turtle onto your fishing rod!'})
            else
             TriggerEvent('NRP-notify:client:SendAlert', source, { type = 'error', text = "You dont have a fishing rod"})
            end
