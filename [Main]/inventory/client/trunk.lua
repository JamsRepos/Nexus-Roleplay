        ...
        if totalAmount + data.number <= maxCapacity[vehicleType].item then
         ...
         local count = tonumber(data.number)
          ...
          if data.item.q >= data.number then
            ...
            TriggerServerEvent("core:log", tostring("[TRUNK] "..GetPlayerName(PlayerId()).."("..PlayerId()..") put "..data.number.."x "..data.item.name.."("..data.item.id..") into the vehicle: "..vehicle_plate), "item")
          else
    ...
    function(data, cb)
        ...
        if type(data.number) == "number" and math.floor(data.number) == data.number then
         ...
         if data.item.q >= data.number then
            ...
            exports['NRP-notify']:DoHudText('inform', 'You can not take out more than what is in the vehicle!')
         end
        end
