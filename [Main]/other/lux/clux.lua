	...
	while true do
			...
			if IsPedInAnyVehicle(playerped, false) then	
				...
				if GetPedInVehicleSeat(veh, -1) == playerped then
					...
					--- IS EMERG VEHICLE ---
					if GetVehicleClass(veh) == 18 or DecorGetBool(GetPlayerPed(-1), "isOfficer") or DecorGetBool(GetPlayerPed(-1), "isParamedic") then
...
-- Unmarked Lights 
local emsLights = false
local runningRed = false 

Citizen.CreateThread(function()
 while true do
  Wait(5)
  if DecorGetBool(GetPlayerPed(-1), "isParamedic") then
   if not IsPedInAnyPoliceVehicle(GetPlayerPed(-1)) then 
   	if IsControlJustPressed(0, 311) then 
   	 local vehicle = GetVehiclePedIsIn(GetPlayerPed(-1), false)
	 if not emsLights then 
	  SetVehicleMod(vehicle, 14, 1)	 
	  SetVehicleNeonLightEnabled(vehicle, 0, true)
	  SetVehicleNeonLightEnabled(vehicle, 1, true)
	  SetVehicleNeonLightEnabled(vehicle, 2, true)
	  SetVehicleNeonLightEnabled(vehicle, 3, true)
	  emsLights = true
      TriggerEvent('chatMessage', '^2Lights Enabled')
     else 
	  SetVehicleMod(vehicle, 14, -1)	 
	  ...
	  emsLights = false
      TriggerEvent('chatMessage', '^2Lights Disabled')
     end
    end
   end
  end
 ...
 while true do
  ...
  if emsLights then
   if IsPedInAnyVehicle(GetPlayerPed(-1), false) then 
   	local vehicle = GetVehiclePedIsIn(GetPlayerPed(-1), false)
    if runningRed then 
     SetVehicleNeonLightsColour(vehicle, 255, 0, 4)
    else 
     SetVehicleNeonLightsColour(vehicle, 255, 255, 255)
    end
    runningRed = not runningRed
   end
  end
 end
end)