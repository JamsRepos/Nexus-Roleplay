...
function getUser(username, password, cb)
  exports['GHMattiMySQL']:QueryResultAsync("SELECT id, username as author, avatar_url as authorIcon FROM twitter_accounts WHERE twitter_accounts.username = @username AND twitter_accounts.password = @password", {
    ['@username'] = username,
    ['@password'] = password
  }, function (data)
    cb(data[1])
...
function TwitterPostTweet (username, password, message, sourcePlayer, realUser)
  getUser(username, password, function (user)
    ...
    exports['GHMattiMySQL']:QueryAsync("INSERT INTO twitter_tweets (`authorId`, `message`, `realUser`) VALUES(@authorId, @message, @realUser);", {
      ['@authorId'] = user.id,
      ['@message'] = message,
      ['@realUser'] = realUser
    }, function (id)
      exports['GHMattiMySQL']:QueryResultAsync('SELECT * from twitter_tweets WHERE authorId = @id', {
        ['@id'] = user.id
      }, function (tweets)
        tweet = tweets[1]
...
function TwitterToogleLike (username, password, tweetId, sourcePlayer)
  getUser(username, password, function (user)
    ...
    exports['GHMattiMySQL']:QueryResultAsync('SELECT * FROM twitter_tweets WHERE id = @id', {
      ['@id'] = tweetId
    }, function (tweets)
      if (tweets[1] == nil) then return end
      local tweet = tweets[1]
      exports['GHMattiMySQL']:QueryResultAsync('SELECT * FROM twitter_likes WHERE authorId = @authorId AND tweetId = @tweetId', {
        ['authorId'] = user.id,
        ['tweetId'] = tweetId
      }, function (row) 
        if (row[1] == nil) then
          exports['GHMattiMySQL']:QueryAsync('INSERT INTO twitter_likes (`authorId`, `tweetId`) VALUES(@authorId, @tweetId)', {
            ['authorId'] = user.id,
            ['tweetId'] = tweetId
          }, function (newrow)
            exports['GHMattiMySQL']:QueryResultAsync('UPDATE `twitter_tweets` SET `likes`= likes + 1 WHERE id = @id', {
              ['@id'] = tweet.id
            }, function ()
          ...
          exports['GHMattiMySQL']:QueryResultAsync('DELETE FROM twitter_likes WHERE id = @id', {
            ['@id'] = row[1].id,
          }, function (newrow)
            exports['GHMattiMySQL']:QueryResultAsync('UPDATE `twitter_tweets` SET `likes`= likes - 1 WHERE id = @id', {
              ['@id'] = tweet.id
...
function TwitterCreateAccount(username, password, avatarUrl, cb)
  exports['GHMattiMySQL']:QueryAsync('INSERT IGNORE INTO twitter_accounts (`username`, `password`, `avatar_url`) VALUES(@username, @password, @avatarUrl)', {
    ['username'] = username,
    ['password'] = password,
    ['avatarUrl'] = avatarUrl
  }, cb)
...
AddEventHandler('gcPhone:twitter_changePassword', function(username, password, newPassword)
  ...
  getUser(username, password, function (user)
    if user == nil then
      ...
      exports['GHMattiMySQL']:QueryAsync("UPDATE `twitter_accounts` SET `password`= @newPassword WHERE twitter_accounts.username = @username AND twitter_accounts.password = @password", {
        ['@username'] = username,
        ['@password'] = password,
...
AddEventHandler('gcPhone:twitter_setAvatarUrl', function(username, password, avatarUrl)
  local sourcePlayer = tonumber(source)
  exports['GHMattiMySQL']:QueryAsync("UPDATE `twitter_accounts` SET `avatar_url`= @avatarUrl WHERE twitter_accounts.username = @username AND twitter_accounts.password = @password", {
    ['@username'] = username,
    ['@password'] = password,
    ['@avatarUrl'] = avatarUrl
  }, function (result)
