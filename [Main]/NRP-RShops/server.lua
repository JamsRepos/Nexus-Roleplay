...
AddEventHandler('robberies:start', function(name, loc)
  if not storeRobbery then
   ...
   TriggerClientEvent('nrp:dispatch:notify', source, '10-90', json.encode({{robberyMessage="Cameras have spotted a store robbery"}}))
...
AddEventHandler('robberies:end', function(name, loc, pay)
 ...
 TriggerClientEvent('nrp:dispatch:notify', source, '10-90', json.encode({{robberyMessage="Cameras have spotted the register is clean"}}))
...
AddEventHandler('vangelicoRobbery:start', function(loc)
 if not vangelicoRobbery then 
  ...
  TriggerClientEvent('nrp:dispatch:notify', source, '10-90', json.encode({{robberyMessage="Cameras have spotted a jewelry robbery"}}))
  TriggerClientEvent('robberies:addBlip', -1, loc)
