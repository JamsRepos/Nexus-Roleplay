...
function FactorFunction(pos)
    if not pos then
        ...
        if factor == 0.5 and calm then
            calm = false
            exports['NRP-notify']:DoHudText('inform', "You are frustrated")
        end
        TriggerEvent("factor:restore")
    else
        if factor > 1.0 or factor == 0.9 then
            if not calm then
                exports['NRP-notify']:DoHudText('inform', "You are calm again")
                calm = true
            end            
            factor = 1.0
            return
...
RegisterCommand('testtaskbar', function()
    ...
    --local finished = exports["skillbar"]:taskBar(4000,math.random(5,15))
    local finished = taskBar(4000,math.random(5,15))
    if finished ~= 100 then
        exports['NRP-notify']:DoHudText('inform', 'You\'re trash at this...')
        --exports['NRP-notify']:DoHudText('inform', 'Failed', 2)
    else
        exports['NRP-notify']:DoHudText('inform', 'Finished, now here\'s a cookie! xD')
