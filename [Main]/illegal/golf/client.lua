	...
	while true do
		...
		if distance < 1.5 then
			...
			if golfplaying then
				...
				DisplayHelpText("Press ~g~~INPUT_CONTEXT~~s~ to start golf.")
			end
			if (IsControlJustReleased(1, 38)) then
				...
				if (golfplaying) then
					...
					Wait(3000)
					exports['NRP-notify']:DoHudText('inform', " Hold E to swing, A-D to rotate, Y to swap club.")
				end
			end
...
function endShot()
	...
	if distance < 1.5 then
		exports['NRP-notify']:DoHudText('inform', "You got the ball with in range!")
		totalgolfstrokes = golfstrokes + totalgolfstrokes
		golfstrokes = 0
		ballstate = 1
	end
	if golfstrokes > 12 then
		exports['NRP-notify']:DoHudText('inform', "You took too many shots.")
		totalgolfstrokes = golfstrokes + totalgolfstrokes
		golfstrokes = 0
		ballstate = 1
	end
