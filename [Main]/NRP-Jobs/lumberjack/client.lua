...
Citizen.CreateThread(function()
 ...
 while true do
  ...
  -- End
   ...
   if(GetDistanceBetweenCoords(GetEntityCoords(GetPlayerPed(-1)), -599.592, 5354.819, 69.499, true) < 75) then
  ...
  if xzurvanticheat == true then 
   if(GetDistanceBetweenCoords(GetEntityCoords(GetPlayerPed(-1)), 1206.432, -1265.043, 35.226, true) < 40) and IsPedInAnyVehicle(GetPlayerPed(-1)) then
     drawTxt('Park The Trailer & Press ~g~ [E] ~w~ To Drop It Off')
     if IsControlJustPressed(0, 38) then
       LogsDelivered()
       xzurvanticheat = false
     end
   end
  end 
  ...
  if WarMenu.IsMenuOpened('lumberjack_boss') then
   if WarMenu.Button('Go On Duty') then
    ...
    exports['NRP-notify']:DoHudText('inform', "Go Chop Down Some Trees! After Come Back To the Right Side of The Yard And Load Them Up")
    ...
    for i,v in ipairs(trees) do
     ...
     if (dist < 1.2) then
      if (hatchetarmed) then
       drawTreeCutText("[E]~w~ to harvest this tree.")
...
AddEventHandler('lumberjack:loadlogs', function()
  ...
  Citizen.CreateThread(function() 
   ...
   TriggerEvent("essence:admin", '100')
...
function PaymentNeeded()
	...
	if IsPedInAnyVehicle(ped) and IsVehicleModel(GetVehiclePedIsIn(ped, false), GetHashKey("phantom")) and needspayment then
		local lumberPay = math.random(7000,8000)
