...
function BoxDelivery()
 ...
 Citizen.CreateThread(function() 
  ...
  SetJobBlip(16.01, 6281.12, 31.70)
 ...
 stage = 1
 jobname = 'Box'
 earnedmoney = math.random(2500, 3200)
end
...
Citizen.CreateThread(function()
 while true do
  Citizen.Wait(5)
  if DecorGetInt(GetPlayerPed(-1), "Job") == 11 and jobname == 'Box' then
   if stage == 1 then 
    if(GetDistanceBetweenCoords(GetEntityCoords(GetPlayerPed(-1)), 16.01, 6281.12, 31.70, true) < 100) and not loaded then
      loaded = true
      RequestModel(GetHashKey('Trailers2'))
      Citizen.CreateThread(function() 
       while not HasModelLoaded(GetHashKey('Trailers2')) do  
        Citizen.Wait(0)  
       end
       trailer = CreateVehicle(GetHashKey('Trailers2'), 16.01, 6281.12, 31.70, 0, true, false)
       SetEntityHeading(trailer, 116.720)
       SetVehicleOnGroundProperly(trailer)
      end)
    elseif(GetDistanceBetweenCoords(GetEntityCoords(GetPlayerPed(-1)), 16.01, 6281.12, 31.70, true) < 100) and loaded then
      DrawMarker(2,  16.01, 6281.12, 31.70, 0, 0, 0, 0, 0, 0, 3.0,3.0,3.0, 16,197,244, 200, 0, 0, 2, 0, 0, 0, 0)
    end
    if IsVehicleAttachedToTrailer(truck) and loaded then
      ...
      SetJobBlip(-607.23, -891.12,  25.01)
      stage = 2
    end 
   elseif stage == 2 then
    DrawMarker(2,  -607.23, -891.12,  25.01, 0, 0, 0, 0, 0, 0, 2.0,2.0,2.0, 16,197,244, 200, 0, 0, 2, 0, 0, 0, 0)
    if(GetDistanceBetweenCoords(GetEntityCoords(GetPlayerPed(-1)), -607.23, -891.12,  25.01, true) < 10) then
     nearAThing = true
     if IsVehicleAttachedToTrailer(truck) then
      DrawText3Ds(-607.23, -891.12,  25.01,'~g~[E]~w~ Deliver The Container')
   ...
   elseif stage == 3 then
    ...
    if(GetDistanceBetweenCoords(GetEntityCoords(GetPlayerPed(-1)), 1204.956, -1264.899, 37.227, true) < 5) then
     ...
     if IsControlJustPressed(0, 38) then
      ...
      stage = 0 
      CheckHealth()
      TriggerServerEvent('jobs:paytheplayer', earnedmoney, 'Trucking: Box')
      jobname = ''
      loaded = false
     end
    else
