...
local toggle_lock_on = 0 -- control id to lock onto a vehicle with the camera or unlock from vehicle (with or without camera). Default is INPUT_SPRINT (spacebar)
...
Citizen.CreateThread(function()
	while true do
        Citizen.Wait(0)
		if IsPlayerInPolmav() or IsPlayerInmaverick() then
			...
			if target_vehicle and GetPedInVehicleSeat(heli, -1) == lPed then
				...
				if IsControlJustPressed(0, toggle_lock_on) or target_distance > maxtargetdistance and not disableLocked then
			...
			while helicam and not IsEntityDead(lPed) and (GetVehiclePedIsIn(lPed) == heli) and IsHeliHighEnough(heli) do
				...
				if locked_on_vehicle then
					...
					if DoesEntityExist(vehicle_detected) then
						...
						if IsControlJustPressed(0, toggle_lock_on) and not disableLocked then
							PlaySoundFrontend(-1, "SELECT", "HUD_FRONTEND_DEFAULT_SOUNDSET", false)
...
function IsPlayerInPolmav()
	...
	return IsVehicleModel(vehicle, polmav_hash) or IsVehicleModel(vehicle, polmav2_hash)
end
