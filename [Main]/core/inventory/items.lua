...
AddEventHandler('items:smoke', function()
	if not DoesEntityExist(GetPlayerPed(-1)) then -- Check to see if the player exists
		return false
	end

	if IsPedInAnyVehicle(GetPlayerPed(-1)) then -- If the player is in a vehicle
		exports['NRP-notify']:DoHudText('error', "Exit the vehicle in order to smoke safely.")
		return false
	end
  ...
  if IsAnyVehicleNearPoint(coords.x, coords.y, coords.z, 5.0) then
    ...
    if DoesEntityExist(vehicle) then
      ...
      Citizen.CreateThread(function()
        ...
        ClearPedTasksImmediately(ped)
        exports['NRP-notify']:DoHudText('inform', "Vehicle Cleaned")
      end)
    end
  end
end)
