...
AddEventHandler("playerConnecting", function(name, reject, d)
    ...
    if not currentDiscordID then
		d.update(Config.NoDiscord)
		Wait(750)
		currentDiscordID = "discord:000000000000000000"
    end
...
function ProccessQueue(steamID, discordID, d, _source)
	...
	PerformHttpRequest("https://discord.com/api/guilds/" .. Config.DiscordServerID .. "/members/"..string.sub(discordID, 9), function(err, text, headers) 
		if text ~= nil then
...
function GetMessage()
	...
	if #playersInfo > 1 then
		SetConvar("Queue", "Queue Enabled | Count: " .. tostring(#playersInfo))
	else
		SetConvar("Queue", "Queue Enabled")
	end
