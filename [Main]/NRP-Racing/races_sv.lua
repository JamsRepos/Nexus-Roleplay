        ...
        if source == race.owner and time < race.startTime then
            ...
            for _, player in pairs(race.players) do
                ...
                TriggerEvent('core:getPlayerFromId', source, function(user)
                    user.addMoney(race.amount)
                    TriggerClientEvent('NRP-notify:client:SendAlert', source, { type = 'success', text = "You have been refunded $"..race.amount.." for race cancellation."})
                end)
...
AddEventHandler("StreetRaces:joinRace_sv", function(index)
    TriggerEvent('core:getPlayerFromId', source, function(user)
        ...
        local playerMoney = user.getMoney(source)
        local playerBank = user.getBank(source)
        if playerMoney >= amount then
            -- Deduct money from player and add to prize pool
            user.removeMoney(amount)
            TriggerClientEvent('NRP-notify:client:SendAlert', source, { type = 'success', text = "You have been charged the join fee of $"..amount.."!"})
        ...
        elseif playerBank >= amount then
            ...
            TriggerClientEvent('NRP-notify:client:SendAlert', source, { type = 'error', text = "You do not have enough money to enter!"})
        end
    end)
end)
    ...
    for index, player in pairs(players) do
        if source == player then 
            ...
            if race.finishTime == 0 then
                ...
                TriggerEvent('core:getPlayerFromId', source, function(user)
                    user.addMoney(race.prize)
                    TriggerClientEvent('NRP-notify:client:SendAlert', source, { type = 'success', text = "Congratulations you won $"..race.prize.."!"})
                end)
