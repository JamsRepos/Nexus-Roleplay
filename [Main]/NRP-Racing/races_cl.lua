...
RegisterCommand("race", function(source, args)
    ...
    elseif args[1] == "cancel" then
        ...
        TriggerEvent('chatMessage', "[StreetRaces]", {255, 0, 0}, " 1. If you already have created a race, skip to Step 5.")
        TriggerEvent('chatMessage', "[StreetRaces]", {255, 0, 0}, " 2. /race record - Starts the recording of the race.")
        TriggerEvent('chatMessage', "[StreetRaces]", {255, 0, 0}, " 3. Open GPS and mark out race.")
        TriggerEvent('chatMessage', "[StreetRaces]", {255, 0, 0}, " 4. /race save <name> - Give it a unique name.")
        TriggerEvent('chatMessage', "[StreetRaces]", {255, 0, 0}, " 5. /race load <name>")
        TriggerEvent('chatMessage', "[StreetRaces]", {255, 0, 0}, " 6. /race start <entry fee> <delay before race starts>")
    end
end)
        ...
        if IsPedInAnyVehicle(player, false) then
            ...
            if raceStatus.state == RACE_STATE_RACING then
                ...
                if config_cl.hudEnabled then
                    ...
                    Draw2DText(config_cl.hudPosition.x, config_cl.hudPosition.y, ("~g~%02d:%06.3f"):format(timeMinutes, timeSeconds), 0.7)
                    local checkpoint = race.checkpoints[raceStatus.checkpoint]
                    ...
                    Draw2DText(config_cl.hudPosition.x, config_cl.hudPosition.y + 0.04, ("~g~CHECKPOINT %d/%d (%dm)"):format(raceStatus.checkpoint, #race.checkpoints, checkpointDist), 0.5)
                end
            ...
            elseif raceStatus.state == RACE_STATE_JOINED then
                ...
                elseif count <= config_cl.freezeDuration then
                    ...
                    Draw2DText(0.5, 0.4, ("~g~%d"):format(math.ceil(count/1000.0)), 3.0)
                    ...
                    Draw3DText(race.startCoords.x, race.startCoords.y, zCoord+1.0, ("Race for ~g~$%d~w~ starting in ~g~%d~w~s"):format(race.amount, math.ceil(count/1000.0)))
            ...
            -- Player is not in a race
                ...
                for index, race in pairs(races) do
                    ...
                    if proximity < config_cl.joinProximity and currentTime < race.startTime then
                        ...
                        Draw3DText(race.startCoords.x, race.startCoords.y, zCoord+1.0, ("Race for ~g~$%d~w~ starting in ~g~%d~w~s"):format(race.amount, count))
