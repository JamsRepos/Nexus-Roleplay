      ...
      if(GetDistanceBetweenCoords(GetEntityCoords(GetPlayerPed(-1)), v.x, v.y, v.z, true) < 4.0) then
       ...
       if IsControlJustPressed(0, 38) then
        if IsPedInAnyVehicle(GetPlayerPed(-1), false) then
         ...
         currentgarage = {id=v.id, x=v.x, y=v.y, z=v.z}
         if DecorGetInt(GetPlayerPed(-1), "Faction") == 13 then
...
-- WarMenu.CreateLongMenu('police_breakroom1', "Fridge")
 ...
 while true do
  ...
  if WarMenu.IsMenuOpened('police_garage') then
    if selectedtype == 1 and WarMenu.ComboBox('Vehicle', {'Mustang', '2018 Charger', 'Explorer', 'Tahoe', 'Crownvick', 'Impala', 'Prison Bus', 'Motorbike', 'Riot Van'}, currentveh, selectedveh, function(veh)
      currentveh = veh
      selectedveh = currentveh
      if currentveh == 1 then vehicle = '2015polstang'
      ...
      elseif currentveh == 9 then vehicle = 'riot'
      end
     end) then
...
RegisterCommand("crt", function(source, args, raw)
  if DecorGetInt(PlayerPedId(), 'Faction') == 5 then
      if isCRT then
        ...
        if exports['core']:GetItemQuantity(170) >= 1 then
