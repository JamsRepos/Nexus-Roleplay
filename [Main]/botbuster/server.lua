-- Server side events that are abused with TriggerServerEvent
...
AddEventHandler('anticheat:ban', function()
	...
	TriggerEvent("core:getPlayerFromId", source, function(target)
		...
		function(result)
			if result[1]['id'] ~= nil then
				for k,v in ipairs(playerIdentifiers) do
					...
					{['@userid'] = result[1]['id'], ['@identifier'] = v, ['@reason'] = '(Anti-Cheat): Hacking', ['@admin'] = 'Anti-Cheat System'})
				end
			end
		end)
		...
		{['@admin'] = 'Anti-Cheat System', ['@user'] = GetPlayerIdentifier(source), ['@action'] = 'Banned: (Anti-Cheat): Hacking'})
		DropPlayer(source, "You are banned, you may appeal it in Discord | Reason: " .. '(Anti-Cheat): Hacking')
	end)
end)
...
AddEventHandler('anticheat:kick', function()
	...
	TriggerEvent("core:getPlayerFromId", source, function(user)
		...
		exports['GHMattiMySQL']:QueryAsync("INSERT INTO admin_history (`admin`, `user`, `action`) VALUES (@admin, @user, @action)",
		{['@admin'] = 'Anti-Cheat System', ['@user'] = GetPlayerIdentifier(source), ['@action'] = 'Kicked: (Anti-Cheat): Hacking'})
		DropPlayer(source, 'Kicked: (Anti-Cheat): Hacking')
	end)
end)
...
for i, eventName in ipairs(ForbiddenEvents) do
	...
	AddEventHandler(
	  ...
	  function()
		...
		exports['GHMattiMySQL']:QueryAsync("INSERT INTO admin_history (`admin`, `user`, `action`) VALUES (@admin, @user, @action)",
		{['@admin'] = 'Anti-Cheat System', ['@user'] = GetPlayerIdentifier(_source), ['@action'] = 'Banned: (Anti-Cheat): Hacking'})
		DropPlayer(_source, "You are banned, you may appeal it in Discord | Reason: " .. '(Anti-Cheat): Hacking')
	  end
	)
  end
